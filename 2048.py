...
from collections import defaultdict     # collections 提供了一个字典的子类 defaultdict，可以指定key值不存在时，value 的默认值
...
class GameField(object):
    ...
    # 随机生成一个 2 或者 4
    def spawn(self):
        # 从 100 中取一个随机数，如果这个随机数大于89，new_element 等于 4，否则等于2
    ...
    def move_is_possible(self, direction):
        ...
        # 检查矩阵每一行有没有可以右移动的元素。这里只用进行变换，所以矩阵变换之后，不用再变换复原
...
def main(stdscr):
    ...
    def game():
        ...
        if action == 'Exit':
            return 'Exit'
        if game_field.move(action):    #if 成功移动了一步
